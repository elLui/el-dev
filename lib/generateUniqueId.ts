 ...
 *  - value is separated into 3 sections: timestamp, random_pi, random_number;
...
export default function generateUniqueId(): string {
    ...
    const random_pi: number = ( Math.random() * Math.PI * 2 ) - Math.PI;

    // random number within range(-32_000, 64_000)
    const random_number: number = Math.floor( ( Math.random() * ( 64000 - ( -32000 ) ) ) + ( -32000 ) );
